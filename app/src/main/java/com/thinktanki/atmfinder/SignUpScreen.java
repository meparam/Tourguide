...
// End Declaration
    ...
    public void onClick(View v)
    {
        switch (v.getId())
        {
            case R.id.create_new_user_button :

                String contents="ajinkya";
...
//
                ...
                case R.id.reenter_password_sign_up :
                    //String PASSWORD_PATTERN1 = "((?=.*[a-z])(?=.*\\d)(?=.*[A-Z])(?=.*[@#$%!]).{8,40})";
                   // Pattern pattern1= Pattern.compile(PASSWORD_PATTERN1);
                    //Matcher matcher1=pattern1.matcher(s.toString());
                    if(s.toString().equals(""))
                    {
                        reEnterPasswordEditText.setError("Password is empty");
                        counters[5]=0;
                    }
                    else {
